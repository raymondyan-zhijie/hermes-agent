...
# ---------------------------------------------------------------------------
# Governance-protected paths (2026-09-21, fork carry: P2-4 closure).
#
# Hermes 四 bot 治理的三区模型里，「生产区」文件（各 profile 的 SOUL.md / config.yaml
# 与技能 references/）此前**只有 memory 与 skills 走 write_approval 闸门**，
# 直写不受保护（独立审计 P2-4：漂移窗口最长 24h）。
# 这里把它们并入既有 approval 闸门：交互式调用者弹审批（once/session/always），
# 无人类通道的调用者 fail closed。
...
def _governance_protected(resolved: str, home: str) -> bool:
    ...
    try:
        ...
        base = os.path.realpath(os.path.join(home, ".hermes"))
...
def is_write_approval_required(path: str) -> bool:
    ...
    home, resolved = _home_and_resolved(path)
    return resolved in build_write_approval_paths(home) or _governance_protected(resolved, home)
